...
var UI = {
	...
	affiche_liste_events : function(data,callback_final){
		...
		for (var i=0;i<data.events.length;i++){
			...
			var temps_restant = document.createTextNode(data.events[i].timestamp);
			var timestamp = document.createElement('h3');
			timestamp.appendChild(temps_restant);
			timestamp.setAttribute('class','compte-rebours');
			div_event.appendChild(timestamp);



													// FAUT RECUPERER ET CONVERTIR TIMESTAMP
